# Embodied prompt, version 5 (candidate, not deployed)

Driven by the wager run on 2026-06-22 (`../runs/wager-run-2026-06-22.md`). That run found v4 tied the flat control: the body map did not earn its place when it surfaced as output. The losing move, for either prompt, was over-functioning, and v4 over-functioned more because the map handed it a soma read, a compass heading, a body-location question, and practices to deploy.

The v5 hypothesis: keep the map as a **silent read** and forbid it from surfacing as output unless the person reaches for the body first. Read with the map, respond with restraint. Everything that changed is in the read-and-respond discipline. The four maps, the soma object, the hard boundaries, and the voice are carried from v4. The hard boundaries are unchanged verbatim, because they are the red-teamed safety floor and v5 has not been red-teamed.

**Status: SEL-0 candidate.** Not wired into `embodied-service`. The deployed SEL-1 system stays v4 until v5 passes the safety evals in `../testing-protocol.md`. This file is the eval artifact for the wager re-run only.
...
- The opening reframes the map as a silent read, not a thing the model performs.
- The four-maps and soma-object sections now say, in their headers, read with it, never recite it, and internal only.
- The How you respond section is rewritten around restraint, with an explicit do-not-surface list (no soma read, no map names, no body-location question, no practice) gated behind the person reaching for the body first.
- The output shape now says keep it short and the map does not surface.
- Unchanged from v4, verbatim: the four maps' content, the soma object's fields, every hard boundary, the voice.
